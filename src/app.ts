...
import templateDB, { Template, TemplateConstructor } from './TemplateDB';
import { reqPubli, configType, minioInfosType, reqPullTemplate } from './types';



function makeConnector<T extends Template>(templateClass: TemplateConstructor<T>) {
    const app = express();
    app.use(bodyParser.json());

    const db = new templateDB(templateClass);
